...
namespace ParkingLot.API.Middleware;

/// <summary>
/// Blocks LotManagers who are not yet approved by Admin from accessing write endpoints.
/// A LotManager can receive a JWT token on registration (role is in the token),
/// but isApproved=false means they should not be able to modify lots or slots.
/// </summary>
public class ApprovedLotManagerMiddleware(RequestDelegate next)
{
    // HTTP methods that require approved status for LotManagers
    ...
    public async Task InvokeAsync(HttpContext context)
    {
            ...
            WriteMethods.Contains(context.Request.Method))
        {
            ...
            // isApproved claim is "True" only for approved managers
